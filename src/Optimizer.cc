...
* 是用来优化的类，ORB SLAM2中所有优化的方法都存放在这个类中
*/
...
int Optimizer::PoseOptimization(Frame *pFrame)
{
    ...
    for(int i=0; i<N; i++)
    {
        ...
        if(pMP)
        {
			...
			//此处 "<0"即代表单目 单目情况, 也有可能在双目下, 当前帧的左兴趣点找不到匹配的右兴趣点
            ...
            else  // Stereo observation  双目的观测，包括RGBD虚拟出的双目，与单目的区别是观测多了一个kp_ur，由二维变成了三维
            {
                nInitialCorrespondences++;
                pFrame->mvbOutlier[i] = false;
...
{    
	...
	// 步骤2：找到关键帧一级连接的关键帧，加入lLocalKeyFrames中
	...
	// 步骤3：遍历lLocalKeyFrames中关键帧，将它们观测的地图点加入到lLocalMapPoints
	...
	// 步骤6：把局部地图中的关键帧加入顶点 添加顶点：Pose of Local KeyFrame
    for(list<KeyFrame*>::iterator lit=lLocalKeyFrames.begin(), lend=lLocalKeyFrames.end(); lit!=lend; lit++)
    {
        KeyFrame* pKFi = *lit;
        g2o::VertexSE3Expmap * vSE3 = new g2o::VertexSE3Expmap();
	...
	// 步骤7：把能被局部MapPoints观测到，但不属于局部地图的关键帧加入顶点 添加顶点：Pose of Fixed KeyFrame，注意这里调用了vSE3->setFixed(true)。
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        //Set edges
		//步骤8：添加与MapPoint相连的边 对每一对关联的MapPoint和KeyFrame构建边
    ...
    // Check inlier observations 
	//  步骤12：优化后重新计算误差，剔除连接误差比较大的关键帧和地图点 
    for(size_t i=0, iend=vpEdgesMono.size(); i<iend;i++)
    {
        g2o::EdgeSE3ProjectXYZ* e = vpEdgesMono[i];
        MapPoint* pMP = vpMapPointEdgeMono[i];
                                       ...
                                       const map<KeyFrame *, set<KeyFrame *> > &LoopConnections, const bool &bFixScale)
{
	...
	//这表明误差变量为7维，误差项为3维 指定线性方程求解器使用Eigen的块求解器
    ...
    // Set Loop edges
	// 步骤3：添加边：LoopConnections是闭环时因为地图点调整而出现的新关键帧连接关系
    ...
    for(size_t i=0, iend=vpKFs.size(); i<iend; i++)
    {
        ...
        // Loop edges
		// 步骤4.2：步添加在CorrectLoop函数中AddLoopEdge函数添加的闭环连接边（当前帧与闭环匹配帧之间的连接关系）
	...
	// 步骤7：优化得到关键帧的位姿后，地图点根据参考帧优化前后的相对关系调整自己的位置
...
int Optimizer::OptimizeSim3(KeyFrame *pKF1, KeyFrame *pKF2, vector<MapPoint *> &vpMatches1, g2o::Sim3 &g2oS12, const float th2, const bool bFixScale)
{
    ...
    vSim3->_focal_length1[0] = K1.at<float>(0,0); // 光心纵坐标cy
